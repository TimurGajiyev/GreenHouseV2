...
# ONLY CHANGE from pglib-uc/uc_model.jl: Cbc + JuMP 0.19 syntax -> HiGHS + JuMP 1.x,
# and the objective is written out as JSON. No constraint or objective term is touched.
using HiGHS
set_optimizer(m, HiGHS.Optimizer)
set_optimizer_attribute(m, "mip_rel_gap", 0.0)
set_optimizer_attribute(m, "time_limit", 1800.0)
optimize!(m)
...
open(ARGS[2], "w") do io
    JSON.print(io, Dict(
        ...
        "commitment" => Dict(g => [value(ug[g,t]) for t in time_periods] for g in thermal_gens),
        "output" => Dict(g => [value(pg[g,t]) + data["thermal_generators"][g]["power_output_minimum"]*value(ug[g,t])
                               for t in time_periods] for g in thermal_gens),
    ), 1)
end
println("objective ", objective_value(m), "  status ", termination_status(m))
